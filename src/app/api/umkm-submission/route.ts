...
export async function POST(req: Request) {
  try {
    ...
    // BODY
    // =========================
    const { umkm_template_file } =
      await req.json()

    if (!umkm_template_file) {
      return NextResponse.json(
        {
          error:
            "File template wajib diisi",
        },
        { status: 400 }
      )
    }
    ...
    if (role === "partner") {
      ...
      if (fetchError) {
        return NextResponse.json(
          {
            error:
              "Gagal ambil data partner profile",
          },
          { status: 500 }
        )
      }
      ...
      if (fetchError) {
        return NextResponse.json(
          {
            error:
              "Gagal ambil data member profile",
          },
          { status: 500 }
        )
      }
      ...
      return NextResponse.json(
        {
          error:
            "Role tidak diizinkan untuk submit UMKM",
        },
        { status: 403 }
      )
    }
    ...
    const FROM_EMAIL =
      process.env
        .RESEND_FROM_EMAIL ||
      "Hablun System <onboarding@resend.dev>"

    const TO_EMAILS =
      process.env.HABLUN_EMAIL
        ...
        <div style="font-family:Arial,sans-serif;padding:20px;background:#f6f6f6">
          ...
          <div style="background:#fff;padding:24px;border-radius:14px">
            ...
            <p style="margin-top:8px;color:#666">
              Ada submission baru dari jalur
              <strong>
                ${role.toUpperCase()}
              </strong>
            </p>
            ...
            <table style="width:100%;border-collapse:collapse">
              ...
              <tr>
                <td style="padding:10px 0;color:#666;width:180px">
                  Nama Bisnis
                </td>
    ...
    return NextResponse.json({
      success: true,
      message:
        "Submission & email berhasil dikirim",
      email_id: data?.id,
    })
  } catch (err) {
    console.error(
      "UMKM submission error:",
      err
    )
